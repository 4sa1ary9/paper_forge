...
**当前项目已经完成 Step 27 的 scaffold 闭环、LLM Query Planner、OpenAI-compatible LLM 执行层、PDF 文本证据提取、paragraph / chunk 级证据切分、本地 evidence search、深度笔记准备度计划、保守版深度笔记写入 MVP、ai-paper-reader Prompt Pack、ai-paper-reader Note Generation、术语证据 MVP、疑难点证据 MVP、代码映射证据 MVP 和面试项目适配度评估 MVP，但还没有完成文档中描述的最终研究包目标。**

最近核对日期：2026-05-25。当前状态是 Step 27 completed；核心 MVP、Extension 0、LLM 执行层、Extension 1 和本地检索 MVP 已闭环，后续建议让 ai-paper-reader note generation 使用 evidence chunks。

更准确的表述是：

> PaperForge Agent 当前已经跑通单篇论文研究包的确定性 workflow、query plan、LLM-backed ai-paper-reader note、文件产物骨架、PDF 文本证据图、paragraph / chunk 级证据图、本地 evidence search、深度笔记 readiness gate、TL;DR / Paper Overview / Background and Motivation / Core Method / Experiments / Limitations / Deep Q&A / Practical Takeaways 的保守证据草稿、ai-paper-reader handoff prompt、带页码证据的术语候选条目、由证据草稿派生的疑难点候选、本地代码目录扫描得到的文件级代码映射候选，以及面试项目适配度评估；但完整深度论文理解、术语自动解释、完整疑难点分析、语义/向量检索、行级或语义级代码映射、完整项目方案仍然是后续规划。
...
| uv 环境和 pytest 基线 | 已完成 | 当前测试基线为 `81 passed`。 |
...
| ai-paper-reader Note Generation | 已完成 MVP | 配置 LLM 后生成 `notes/ai-paper-reader-note.md`，并保存 generation prompt 方便复盘。 |
...
| Chunk-grounded LLM generation | 未实现 | ai-paper-reader、terminology 和 doubts 仍需改成优先使用 `notes/evidence-chunks.md`。 |
...
```text
推荐优先级：Step 28: ai-paper-reader Note 使用 Evidence Chunks
```

这样可以让现有 LLM note generation 优先引用 chunk id，减少只依赖页码级 excerpt 的粗糙生成。
...
远端仓库读取或 clone 仍然需要用户再次确认。Chunk-grounded LLM 生成、语义/向量检索、多篇论文批处理和前后端产品化属于后续扩展路线图；详见 `docs/EXTENSION_ROADMAP.md`。
